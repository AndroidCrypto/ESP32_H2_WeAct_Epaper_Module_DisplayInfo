/*
  The sketch is showing how to connect a WeAct 2.9-inch EPaper Module to an ESP32-H2 Supermini
  microcontroller.
...
// Connections for ESP32-H2 Supermini, trying to have most ADC pins left available
static const uint8_t EPD_BUSY = 24; // to EPD BUSY 24 is labled as 'TX'
static const uint8_t EPD_CS = 14;   // to EPD CS
static const uint8_t EPD_RST = 12;  // to EPD RST
static const uint8_t EPD_DC = 13;   // to EPD DC

static const uint8_t EPD_SCK = 23;   // to EPD CLK 23 is labled as 'RX'
...
/*
Wiring of the display to the ESP32-H2 Supermini device. Please double-check the pins on the PCB
...
/*
  Pins to avoid on an ESP32-H2 Supermini board:
...
void setup() {
  ...
  while (!Serial)
    ;
  Serial.println("Esp32-H2 Supermini WeAct 2.9-inch EPaper Module GxPD2 library DisplayInfo v03");
